...
class Blockchain98Handler extends TransactionHandler {
  ...
  apply (transactionProcessRequest, context) {
    ...
    .then((update) => {
      ...
      const createAccount = (username, publicKey) => {
        ...
        .then((accountValue) => {
          ...
          return bC98State.setAccount(username, publicKey)
        })
      }
      ...
      const createProject = (name, description, goal, publicKey) => {
        if (isNaN(Number(goal))) {
          throw new Error('The goal is not valid!')
        }
        return bC98State.getMessage(`${publicKey}:${name}`, 'Project')
          .catch((err) => {
            const message = err.message ? err.message : err
            logger.error(`getProject in blockchain is not responding!: ${message}`)
            throw new Error(`getProject in blockchain is not responding!: ${message}`)
          })
          .then((projectValue) => {
            if (projectValue && projectValue.publickey !== undefined) {
              logger.error('Project Already exists!!')
              throw new Error('Project Already exists!!')
            }
            logger.info('Creating Project : ' + name + ' for : ' + publicKey)
            return bC98State.setProject(name, description, goal, publicKey)
          })
      }

      const donateProject = (project, amount, publicKey) => {
        if (isNaN(Number(amount))) {
          throw new Error('The goal is not valid!')
        }
        logger.info('Donating to Project : ' + projec)
        return bC98State.setDonation(project, amount, publicKey);
        // return bC98State.getMessage(project, 'Project')
        //   .catch((err) => {
        //     const message = err.message ? err.message : err
        //     logger.error(`getProject in blockchain is not responding!: ${message}`)
        //     throw new Error(`getProject in blockchain is not responding!: ${message}`)
      ...
      switch (action) {
        ...
        case 'CreateProjectAction':
          if (!update && !update.createProject) {
            logger.error('update does not have "createProject" field!')
            throw new Error('update does not have "createProject" field!')
          }
          const { name, description, goal } = update.createProject
          actionPromise = createProject(name, description, goal, userPublicKey);
          break;

        case 'DonateProjectAction':
          if (!update && !update.donateProject) {
            logger.error('update does not have "donateProject" field!')
            throw new Error('update does not have "donateProject" field!')
          }
          const { project, amount:donationAmount } = update.donateProject
          actionPromise = donateProject(project, donationAmount, userPublicKey);
          break;
        default :
